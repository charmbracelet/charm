...
func (me *DB) StashCount() (int, error) {
	var c int
	r := me.db.QueryRow(sqlCountStashes)
	err := r.Scan(&c)
	if err != nil {
		return 0, err
	}
...
func (me *DB) CreateDB() error {
	log.Printf("Creating DB on host %s\n", me.host)
	return me.wrapTransaction(func(tx *sql.Tx) error {
...
func (me *DB) updateUser(tx *sql.Tx, charmID string, name string) error {
	_, err := tx.Exec(sqlUpdateUser, charmID, name)
	return err
}
...
func NewDB(host string, port string, dbName string, user string, pass string) *DB {
	var err error
	var conn string
	db := &DB{}
	if host == "db" || host == "localhost" {
		...
		conn = fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s sslmode=require", host, port, user, pass, dbName)
	}
	db.db, err = sql.Open("postgres", conn)
	if err != nil {
		panic(err)
	}
